...
int main(int argc, char *argv[])
{
  ...
  std::string binary_file = getBoardBinaryFile("kernel", devices[device]);
  printf("Using AOCX: %s\n", binary_file.c_str());
  cl_program program = createProgramFromBinary(context, binary_file.c_str(), &devices[device], num_devices);
	...
	printf("local_work_size: %d", local_work_size); 

	uint32_t *host_bit_values_ = new uint32_t[nr_bit_words * prog_opts.nr_threads];

	while (1) {
		if ((err = clEnqueueNDRangeKernel(queue, kernel, 1,	NULL, &work_size,	&local_work_size, 0, NULL, NULL)) != CL_SUCCESS){
			printf("%d\n", err);
